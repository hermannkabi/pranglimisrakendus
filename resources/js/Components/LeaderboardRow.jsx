import { showPublicName } from "@/utils/display_name";
...
export default function leaderboardRow({auth, place, points, index, player, user, playedToday=false, onRemove=null, customLink=null}){
    ...
    return (
        <a style={{all:"unset", cursor:"pointer"}} href={onRemove ? null : (customLink ?? ("/profile/"+user.id))}>
            <div className="section clickable" style={{position:"relative", marginBlock:"8px", display:"flex", flexDirection:"row", justifyContent:"space-between", alignItems:"center"}}>
                <div style={{display:"flex", flexDirection:"row", alignItems:"center"}}>
                    ...
                    <SizedBox width={12} />
                    {showPublicName(auth.user, user) && <p style={{fontSize:"20px", fontWeight:"bold"}}>{user.public_name}</p> }
                    {!showPublicName(auth.user, user) && <TwoRowTextButton upperColor={player ? "rgb(var(--primary-color))" : null} lowerColor={player ? "rgba(var(--primary-color), 0.65)" : null} showArrow={false} capitalizeLower={true} capitalizeUpper={true} upperText={user.eesnimi} lowerText={user.perenimi} />}
                    <SizedBox width="4px" />
                    {onRemove && <i onClick={onRemove} style={{position:"absolute", right:"16px", cursor:"pointer"}} className="material-icons">close</i> }
