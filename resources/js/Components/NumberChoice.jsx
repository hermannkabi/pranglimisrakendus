...
export default function NumberChoice({defaultValue, id, maxValue=10}){
    ...
    return (
        ...
        <div style={{border:"2px solid rgb(var(--primary-color))", borderRadius:"8px", display:"flex", marginInline:"4px", justifyContent:"space-between"}}>
            <div onClick={subtractFromValue} style={{transition:"background-color 100ms, color 100ms", backgroundColor: "rgb(var(--primary-color))", display:"flex", alignItems:"center", padding:"8px 16px"}}>
                <span style={{color: "white", userSelect:"none"}} className="material-icons">remove</span>
            </div>
            <p style={{padding:"4px 16px", userSelect:"none"}}>{value == 0 ? "-" : value ?? "-"} {value == 0 || value == null ? "" : "min"}</p>
            <div onClick={addToValue} style={{transition:"background-color 100ms, color 100ms", backgroundColor:"rgb(var(--primary-color))", display:"flex", alignItems:"center", padding:"8px 16px"}}>
