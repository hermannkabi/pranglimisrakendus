...
export default function LeaderboardPodiumTile({auth, e, firstPlace=false}){
    return  <div style={{flex:"1", textAlign:"center"}}>
                ...
                <a style={{all:"unset"}} href={"/profile/"+e.user.id}>
                    <div className="section clickable">
                        ...
                        <SizedBox height={["1", "T1", "2", "T2"].includes(e.place) ? "32px" : "12px"} />
                        {!firstPlace && <p style={{textTransform:"capitalize", marginBlock:"4px", display:"inline-flex", alignItems:'center', gap:"4px", color: auth.user.id == e.user.id ? "rgb(var(--primary-color))" : null}}>{e.user.eesnimi} {e.user.perenimi} {e.playedToday && <StreakActiveWidget />} </p>}
