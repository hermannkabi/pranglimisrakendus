        ...
        <div className="" style={{display:'flex', flexWrap:"wrap", justifyContent:"center", alignItems:"center", gap:"16px"}}>
            <div style={{overflow:"hidden"}}>
                <div  className="profile-widget" style={{display:"flex", flexDirection:"row", gap:"16px", alignItems:"center"}}>
                    ...
                    <div className="name-email" style={{textAlign:"start", marginBottom: isPublic || user.email.length <= 0 ? "0" : "8px"}}>
                        ...
                        {!isPublic && user.email.length > 0 && <div style={{display:"flex", flexDirection:"row", alignItems:"center"}}><p translate="no" style={{marginBottom:"0", color:"grey", fontSize:"20px", marginTop:"0"}}>{user == null ? "mari.maasikas@real.edu.ee" : user.email} </p>&nbsp;{user.email_verified_at && <span title="E-posti aadress kinnitatud" style={{color:"rgb(var(--primary-color))", fontSize:"18px", userSelect:"none"}} className="material-icons">verified</span>}</div>}
                    </div>
                </div>
            </div>
        </div>
