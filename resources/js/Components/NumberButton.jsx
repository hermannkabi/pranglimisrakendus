...
export default function NumberButton({content, style, backgroundColor, textColor, lineHeight, fontSize, onClick, keyId = content, icon = false, disabled=false}){
    ...
    return (
        <div onClick={disabled ? null : onClick} className="number-button" style={{display:"flex", alignContent:"center", justifyContent:'center', cursor: disabled ? "not-allowed" : "pointer", backgroundColor: backgroundColor ?? "rgb(var(--section-color), var(--section-transparency))", padding:"16px", margin:"4px", borderRadius:"8px", color:"black", width:"32px", height:"32px", fontFamily:"", opacity:disabled ? "0.5" : "1", ...style}}>
